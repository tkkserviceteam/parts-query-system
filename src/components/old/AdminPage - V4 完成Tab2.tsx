...
export default function AdminPage({ onSwitchToFront }: { onSwitchToFront: () => void }) {
  ...
  const renderDataTab = () => (
    <div>
      <div className={styles.sbox}>
        <div className={styles.sboxTitle}>資料列表 ({parts.length})</div>
        {selectedPartIds.length > 0 && <button onClick={() => { if(confirm('刪除？')) supabase.from('parts').delete().in('id', selectedPartIds).then(() => loadPartsForSub()); }} style={{ color: 'red' }}>🗑 刪除 ({selectedPartIds.length})</button>}
      ...
      <div style={UI_STYLE.overlay as any}>
        <div style={UI_STYLE.modal as any}>
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
            ...
            <button onClick={() => onSave(form)}>儲存</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className={styles.adminPage}>
      <div className={styles.topbar} style={{ display: 'flex', justifyContent: 'space-between', padding: '15px' }}>
        <span style={{ fontWeight: 800, fontSize: '20px' }}>零件料號系統 - 後台</span>
        <button onClick={onSwitchToFront}>← 返回前台</button>
      </div>
